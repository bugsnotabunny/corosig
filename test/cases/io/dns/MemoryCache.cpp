...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: push and pull IPv4 addresses") {
  ...
  dns::CachePushTransaction transaction{
      .name = "example.com", .expire_at = expires_at, .ipv4s = addrs};
  ...
  COROSIG_REQUIRE(out_addrs[0].address == *Ipv4Addr::parse("192.168.1.1"));
  COROSIG_REQUIRE(out_addrs[0].expires_at == expires_at);
  COROSIG_REQUIRE(out_addrs[1].address == *Ipv4Addr::parse("192.168.1.2"));
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: push and pull IPv6 addresses") {
  ...
  auto expires_at = SteadyClock::now() + std::chrono::seconds{3600};
  dns::CachePushTransaction transaction{
      .name = "example.com", .expire_at = expires_at, .ipv6s = addrs};
  ...
  COROSIG_REQUIRE(pull_result.is_ok());
  COROSIG_REQUIRE(pull_result.value() == 2);
  COROSIG_REQUIRE(out_addrs[0].address == *Ipv6Addr::parse("2001:db8::1"));
  COROSIG_REQUIRE(out_addrs[0].expires_at == expires_at);
  COROSIG_REQUIRE(out_addrs[1].address == *Ipv6Addr::parse("2001:db8::2"));
  COROSIG_REQUIRE(out_addrs[1].expires_at == expires_at);
}
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: push and pull mixed IPv4 and IPv6 addresses") {
  ...
  auto expires_at = SteadyClock::now() + std::chrono::seconds{3600};
  dns::CachePushTransaction transaction{
      .name = "example.com", .expire_at = expires_at, .ipv4s = ipv4s, .ipv6s = ipv6s};
  ...
  COROSIG_REQUIRE(out_ipv4s[0].address == *Ipv4Addr::parse("192.168.1.1"));
  COROSIG_REQUIRE(out_ipv4s[0].expires_at == expires_at);
  COROSIG_REQUIRE(out_ipv4s[1].address == *Ipv4Addr::parse("192.168.1.2"));
  ...
  COROSIG_REQUIRE(out_ipv6s[0].address == *Ipv6Addr::parse("2001:db8::1"));
  COROSIG_REQUIRE(out_ipv6s[0].expires_at == expires_at);
  COROSIG_REQUIRE(out_ipv6s[1].address == *Ipv6Addr::parse("2001:db8::2"));
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: fills available output buffer") {
  ...
  COROSIG_REQUIRE(out_addrs[0].address == *Ipv4Addr::parse("192.168.1.1"));
  COROSIG_REQUIRE(out_addrs[1].address == *Ipv4Addr::parse("192.168.1.2"));
  COROSIG_REQUIRE(out_addrs[2].address == *Ipv4Addr::parse("192.168.1.3"));
}
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: pruning removes expired entries") {
  ...
  dns::CachePushTransaction expired_transaction{
  ...
  dns::CachePushTransaction valid_transaction{
      .name = "valid.example.com", .expire_at = valid_at, .ipv4s = addrs};
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: handles multiple entries with different names") {
  ...
  dns::CachePushTransaction transaction1{
      .name = "host1.example.com", .expire_at = expires_at, .ipv4s = addrs1};

  dns::CachePushTransaction transaction2{
      .name = "host2.example.com", .expire_at = expires_at, .ipv4s = addrs2};

  dns::CachePushTransaction transaction3{
      .name = "host3.example.com", .expire_at = expires_at, .ipv4s = addrs3};
  ...
  COROSIG_REQUIRE(out_addrs[0].address == *Ipv4Addr::parse("172.16.0.1"));
  COROSIG_REQUIRE(out_addrs[0].expires_at == expires_at);
}
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: handles multiple address records for same name") {
  ...
  dns::CachePushTransaction transaction1{
      .name = "multi.example.com", .expire_at = expires_at, .ipv4s = addrs1};

  dns::CachePushTransaction transaction2{
      .name = "multi.example.com", .expire_at = expires_at, .ipv4s = addrs2};
  ...
  COROSIG_REQUIRE(pull_result.is_ok());
  COROSIG_REQUIRE(pull_result.value() == 2);
  COROSIG_REQUIRE(out_addrs[0].address == *Ipv4Addr::parse("192.168.1.2"));
  COROSIG_REQUIRE(out_addrs[0].expires_at == expires_at);
  COROSIG_REQUIRE(out_addrs[1].address == *Ipv4Addr::parse("192.168.1.1"));
  COROSIG_REQUIRE(out_addrs[1].expires_at == expires_at);
}
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: handles empty address lists") {
  ...
  auto expires_at = SteadyClock::now() + std::chrono::seconds{3600};
  dns::CachePushTransaction transaction{
      .name = ".example.com", .expire_at = expires_at, .ipv4s = {}, .ipv6s = {}};
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: handles long hostnames") {
  ...
  std::string_view long_name = "a.very.long.hostname.that.goes.on.and.on.subdomain.example.com";
  std::array<Ipv4Addr, 1> addrs = {*Ipv4Addr::parse("192.168.1.1")};

  auto expires_at = SteadyClock::now() + std::chrono::seconds{3600};
  dns::CachePushTransaction transaction{.name = long_name, .expire_at = expires_at, .ipv4s = addrs};
...
COROSIG_SIGHANDLER_TEST_CASE("MemoryCache: multiple pulls after prune") {
  ...
  dns::CachePushTransaction expired{
      .name = "test.example.com", .expire_at = expired_at, .ipv4s = expired_addr};

  auto valid_at = now + std::chrono::seconds{3600};
  dns::CachePushTransaction valid{
      .name = "test.example.com", .expire_at = valid_at, .ipv4s = valid_addr};
