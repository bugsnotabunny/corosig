...
write(Reactor &, int fd, std::span<char const>) noexcept;
Fut<size_t, Error<AllocationError, SyscallError>>
write_some(Reactor &, int fd, std::span<char const>) noexcept;
...
Fut<size_t, Error<AllocationError, SyscallError>>
read_some(Reactor &, int fd, std::span<char>) noexcept;
